...
from demo.args_cls import ArgsClass
...
parser.add_argument("--per_digit_tokens", action="store_true")


# args = parser.parse_args()
args = ArgsClass("ecnu-icalk/educhat-base-002-7b", 200, 40, True, False)
...
model = get_specific_model(args.model_path, torch_dtype=torch.float32)
...
tokenizer = transformers.LlamaTokenizer.from_pretrained(args.model_path, torch_dtype=torch.float32)
...
add_special_tokens_count = tokenizer.add_special_tokens({"additional_special_tokens": additional_special_tokens})

print(f"add_special_tokens_count: {add_special_tokens_count}")
...
with gr.Blocks() as demo:
    ...
    submitBtn.click(predict,
                    [user_input, chatbot, max_length, top_p, temperature, history], [chatbot, history],
                    show_progress="full")
